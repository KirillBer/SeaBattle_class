...
class SeaBattleField{
	...
	public:
		...
		int CancelLastMove(){	//Откатывает изменения за последний ход; Возвращает: 0 - ходы закончились; 1 - был выстрел по SHOT, STRIKE, KILL; 2 - было попадание в SHIP; 3 - был подрыв корабля; 4 - был выстрел по EMPTY; 5 - была установка корабля;
			int result = 4;
			if (moves.size())
			{
				...
				for (int index, i = (moves[moves.size() - 1].size() - 1); i >= 0; i--)
	        	{
	        		...
	        		if (moves[moves.size() - 1][i].newState == field_value_mean[4])	//Стала SHIP
	        			result = 5;
					else if (moves[moves.size() - 1][i].newState == field_value_mean[2])	//Стала SHOT
						result = 2;
		...
		static int SaveGameSBF(SeaBattleField &field_1, SeaBattleField &field_2, const char *file_name){
			ofstream file(file_name);	//Открываем файл для записи
			field_1.SaveToFile(file);
			field_2.SaveToFile(file);
		}
		static int LoadGameSBF(SeaBattleField &field_1, SeaBattleField &field_2, const char *file_name){
		...
		int SaveToFile(ofstream &record_file){	//Сохранение в файл; 0 - успешно; 1 - ошибка открытия файла; 2 - ошибка записи
			if (record_file.is_open())
			{
				if (RecordItem(record_file, cols, " ") || RecordItem(record_file, rows, "\n"))
					return RecordFileError(record_file, 2); 
	...
	private:
		static int IsInCharRange(int number){	//0 - в диапазоне; 1 - за диапазоном
			return (number < 0 || number > 255) ? 1 : 0;
		}
		static int IsNewSizeAvailable(int cols_, int rows_){	//0 - доступно; 1 - недоступно
			if ((cols_ < 1 || cols_ > 100000000) || (rows_ < 1 || rows_ > 100000000) || (cols_ > 1000000000 / rows_))
				return 1;
			return 0;
		}
		static int MemoryAllocationError(int error_type){	//Значения error_type: 0 - критически важный объект; 1 - не критически важный объект
			switch(error_type){
				case 0:
					printf("exit 1;\n");
					exit(1);
		...
		static int XYChangesBySide(int x_is_0_y_is_1, int side){	//значения изменения x, y в зависимости от стороны
	...
	public:	//ВАОЛПРВОАРПЛОВАПРЛВОАПРЛВАОП УБЕРИ паблик!!!!!
		static int IntPow(int x, int y){	//Возвращает x в степени y
			int a = 1;
			for(y; y > 0; y--)
					a *= x;
			return a;
	...
	public:
		...
		void *GetField(){	//Отладка. Удалить !!!!
			return field;
		}
		int GetValueOfCellByIndex(int index){	//Возвращает хранящееся значение в клетке
			if (index < 0 || index >= rows * cols)
				return -1;
			return field[index];
		}
		int GetValueOfCell(int col_index, int row_index){	//Возвращает хранящееся значение в клетке
		...
		int ShotTo(int x, int y){	//0 - попадание по SHIP; 1 - подрыв последнего корабля; 2 - выстрел в пустую клетку; 3 - бессмысленный выстрел, попадание в SHOT, STRIKE, KILL; 4 - ошибка координаты; 5 - ошибка значения ячейки
		...
		//Корабли
		void SetShips(){ 	//начинает расстановку всех кораблей поля
			int x = -1, y = -1, side = 0;
			for(int n = GetMaxShipLen(); n > 0; n--)
				for(int i = GetShipsRemainCountOfNLen(n); i > 0; i--)
				{
					printf("Ставлю корабль длины %d, осталось %d\n", n, i);
					while(SetShip(n, side, x, y))
					{
						scanf("%d%d%d", &x, &y, &side);
					}
				}
		}
		int SetShip(int ship_len, int side, int col, int row){ //0 - успешно; 1 - ошибка аргумента; 2 - ошибка установки первой клетки; 3 - ошибка установки в направлении
			if ((col < 0 || col >= cols) || (row < 0 || row >= rows) || (side < 1 || side > 4))
				return 1;
			if (CheckOneBlock(col, row))
				return 2;
			if (ship_len == 1)
			{
				MakeMove();
				AddCellValueChange(cols * row + col, field_value_mean[0], field_value_mean[4]);
				field[cols * row + col] = field_value_mean[4];
				return 0;
			}
			if (!((AvailableSides(col, row, ship_len) / IntPow(10, 4 - side))%10))	//проверяется доступность установки в сторону side
				return 3;
			MakeMove();
			SetInSide(field_value_mean[4], side, ship_len, col, row);
			return 0;
		}
	public:
...
class SeaBattleGame : public SeaBattleField{
	...
	public:
		...
		char DrawSymbol(unsigned char index){	//Отрисовка элемента index поля, в виде игрового символа
			...
			if (cell_value == GetValueOfValueMean(0))
				cout << FieldSymbol[0];
			else if (cell_value == GetValueOfValueMean(1))
				cout << FieldSymbol[1];
			else if (cell_value == GetValueOfValueMean(2))
				cout << FieldSymbol[2];
			else if (cell_value == GetValueOfValueMean(3))
				cout << FieldSymbol[3];
			else if (cell_value == GetValueOfValueMean(4))
				cout << FieldSymbol[4];
		}
		void PrintField(){	//Тут отладка есть!
			cout << "Поле: " << (void *)GetField() << endl; //Отладка. Удалить!!!!
		...
		void PrintFieldValues(){	//Тут отладка есть!
			cout << "Поле: " << (void *)GetField() << endl; //Отладка. Удалить!!!!
			for(int index = 0; index < GetCols() * GetRows(); index++){
		...
		void PrintFieldIndexs(){	//Тут отладка есть!
			cout << "Поле: " << (void *)GetField() << endl; //Отладка. Удалить!!!!
			for(int index = 0; index < GetCols() * GetRows(); index++){
				cout << index;
				if (((index + 1) % GetCols() == 0) && (index > 0))
...
int main() {
	...
	a.LoadFromFileSBF();
	a.PrintFieldValues();
	a.PrintFieldValueMean();
	a.GetShipsRemainCountOfNLen(1);
